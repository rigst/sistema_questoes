...
def _fakes_classificacao(topicos, n_blocos=1):
    ...
    atribuicoes = [
        {"id": qid, "topico": i} for i, t in enumerate(topicos) for qid in t.get("questoes", [])
    ]
    # Cada bloco filtra os IDs que lhe pertencem, então devolver a lista
    # inteira em todos os blocos é inofensivo.
    blocos = [
        _resposta_fake(texto=json.dumps({"atribuicoes": atribuicoes})) for _ in range(n_blocos)
    ]
...
class BaseIATestCase(TestCase):
    def setUp(self):
        ...
        self.prompt = Prompt.objects.create(
            user=self.user, nome="Explicar", texto="Explique a questão."
        )
...
class AnaliseUnicaTests(BaseIATestCase):
    ...
    def test_gerar_analise_pula_ja_prontas(self, get_client):
        ...
        resp = self.client.post(
            reverse("ai:gerar_comentarios"), {"questao_ids": [self.questao.pk]}, follow=True
        )
    ...
    def test_todas_paginas_abrange_disciplina_inteira_sem_questao_ids(self, processar_lote_delay):
        outra = Questao.objects.create(
            disciplina=self.disc,
            numero=2,
            enunciado_md="Outra questão",
            gabarito="B",
        )
        resp = self.client.post(
            reverse("ai:gerar_comentarios"),
            {
                "usar_lote": "1",
                "todas_paginas": "1",
                "disciplina_id": self.disc.pk,
            },
        )
        self.assertEqual(resp.status_code, 302)
        ...
        self.assertEqual(
            ResultadoPrompt.objects.filter(
                prompt=padrao, questao__in=[self.questao, outra]
            ).count(),
            2,
        )
    ...
    def test_sintese_de_topicos_via_batch_para_apos_estourar_o_teto(
        ...
        for i in range(26):
            q = Questao.objects.create(
                disciplina=self.disc, numero=200 + i, enunciado_md=f"Questão extra {i}"
            )
...
class TopicosTests(BaseIATestCase):
        ...
        respostas = [
            _resposta_fake(
                texto=json.dumps(
                    {
                        "topicos": [
                            {"nome": "Tema A", "descricao": "x"},
                            {"nome": "Tema B", "descricao": "y"},
                        ]
                    }
                )
            )
    ...
    def test_consolidacao_recusa_absorver_topico_saudavel(self, get_client):
        ...
        grupos = [
            ...
            {
                "nome": "Competência da Justiça Federal",
                "descricao": "",
                "questoes": list(range(26, 39)),
            },
    ...
    def test_sobras_vao_para_outros_temas_e_lote_e_submetido(self, get_client):
        ...
        get_client.return_value.messages.batches.create.return_value = SimpleNamespace(
            id="batch_teste"
        )
    ...
    def test_questoes_sem_analise_ficam_fora(self, get_client):
        q3 = Questao.objects.create(
            disciplina=self.disc,
            numero=3,
            enunciado_md="Questão sem análise",
        )
        _mock_stream(
            get_client,
            [
    ...
    def test_conteudo_topico_inclui_enunciados_gabaritos_e_analises(self):
        ...
        ResultadoPrompt.objects.create(
            ...
            prompt=self.prompt,
            status=ResultadoPrompt.Status.CONCLUIDO,
            resultado_md="Ponto exclusivo da Q1.",
        )
        ResultadoPrompt.objects.create(
            questao=self.q2,
            prompt=self.prompt,
            status=ResultadoPrompt.Status.ERRO,
            resultado_md="Não deve entrar.",
        )
    ...
    def test_sintese_de_topico_grande_ganha_max_tokens_maior(self):
        ...
        ResultadoPrompt.objects.create(
            ...
            prompt=self.prompt,
            status=ResultadoPrompt.Status.CONCLUIDO,
            resultado_md="Conteúdo extenso. " * 6000,
        )
    ...
    def test_estimar_custo_topicos_cresce_com_mais_conteudo(self):
        q3 = Questao.objects.create(
            disciplina=self.disc,
            numero=3,
            enunciado_md="Enunciado bem maior " * 50,
            gabarito="C",
        )
        ResultadoPrompt.objects.create(
            questao=q3,
