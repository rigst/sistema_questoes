...
# ---------------------------------------------------------------------------
# Mentoria da disciplina (como estudar)
# ---------------------------------------------------------------------------

MENTOR_SYSTEM = (
    'Você é um mentor de estudos para concursos públicos e exames de carreira '
    'no Brasil. Dá orientação prática, honesta e específica ao conteúdo — nada '
    'de motivação genérica. Fala direto com o aluno, em português.'
)


def _instrucao_mentoria(disciplina, user):
    """Monta o contexto real da disciplina: o que mais cai, o que já foi lido
    e como o aluno vem indo na revisão. É isso que personaliza a mentoria."""
    from django.db.models import Count, Q

    from questions.models import LeituraTopico, RespostaRevisao

    topicos = list(
        disciplina.topicos.annotate(n=Count('questoes')).order_by('-n', 'nome')
    )
    total_q = disciplina.questoes.count() or 1
    lidos = set(
        LeituraTopico.objects.filter(user=user, topico__disciplina=disciplina)
        .values_list('topico_id', flat=True)
    )
    perf = {
        p['questao__topico']: p
        for p in RespostaRevisao.objects.filter(
            user=user, questao__disciplina=disciplina,
        ).values('questao__topico').annotate(
            total=Count('id'), acertos=Count('id', filter=Q(correta=True)),
        )
    }

    linhas = []
    for t in topicos[:20]:
        pct = round(t.n / total_q * 100)
        marca = ' · já lido' if t.pk in lidos else ' · não lido'
        p = perf.get(t.pk)
        if p and p['total']:
            marca += f' · revisão: {p["acertos"]}/{p["total"]} acertos'
        linhas.append(f'- {t.nome} — {t.n} questões ({pct}%){marca}')

    tot_resp = sum(p['total'] for p in perf.values())
    tot_ac = sum(p['acertos'] for p in perf.values())
    resumo_perf = ''
    if tot_resp:
        resumo_perf = (
            f'\nNa revisão, o aluno já respondeu {tot_resp} questões, '
            f'com {tot_ac} acertos ({round(tot_ac / tot_resp * 100)}%).'
        )

    return (
        f'Disciplina: {disciplina.nome} (prova: {disciplina.prova.nome}).\n'
        f'Total de questões catalogadas: {total_q}.\n\n'
        f'Temas por incidência (do que mais cai ao que menos cai):\n'
        + '\n'.join(linhas)
        + resumo_perf
        + '\n\nEscreva, em Markdown, um plano de mentoria para ESTA disciplina:\n'
        '- Por onde começar e em que ordem estudar, priorizando o que mais cai '
        'e os temas ainda não lidos.\n'
        '- Onde o aluno está mais fraco (pior aproveitamento na revisão) e como reforçar.\n'
        '- 3 a 6 técnicas de estudo concretas adequadas a esta matéria.\n'
        '- Erros e pegadinhas típicos da disciplina em prova.\n\n'
        'Seja específico ao conteúdo real acima; não reescreva o conteúdo dos '
        'tópicos, foque em COMO estudar. Use títulos "##" e listas curtas. '
        'No máximo ~500 palavras.'
    )


def estimar_tokens_mentoria(disciplina):
    """Estimativa (entrada + saída) de uma geração de mentoria, para a quota."""
    n = disciplina.topicos.count()
    return int(500 + n * 45 + 2500)


def gerar_mentoria_sincrono(disciplina, user, profile=None):
    """Gera as dicas de estudo da disciplina via messages.create e devolve o
    texto + o uso de tokens. Não grava nada — quem chama persiste."""
    client = get_client()
    modelo = getattr(settings, 'AI_MODEL', 'claude-sonnet-5')
    params = {
        'model': modelo,
        'max_tokens': min(getattr(settings, 'AI_MAX_TOKENS', 16000), 3000),
        'system': MENTOR_SYSTEM,
        'messages': [{'role': 'user', 'content': _instrucao_mentoria(disciplina, user)}],
    }
    if _suporta_adaptive(modelo):
        params['thinking'] = {'type': 'adaptive'}
        params['output_config'] = {'effort': getattr(settings, 'AI_EFFORT', 'medium')}
    ...
    if not texto:
        raise IAError(
            f'A IA não retornou texto para a mentoria '
            f'(stop_reason={getattr(resp, "stop_reason", "?")}).'
        )
    it, ot = resp.usage.input_tokens, resp.usage.output_tokens
    custo = custo_usd(it, ot)
    if profile is not None:
        profile.registrar_uso(it, ot, custo)
    return {
        'texto_md': texto, 'modelo': modelo,
        'input_tokens': it, 'output_tokens': ot, 'custo': custo,
    }
