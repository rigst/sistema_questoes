...
class RelatorioTopicosTests(TestCase):
    def setUp(self):
        ...
        self.q1 = Questao.objects.create(
            disciplina=self.disc, numero=1, enunciado_md='Enunciado 1', gabarito='A',
            status=Questao.Status.CONCLUIDA,
        )
        self.q2 = Questao.objects.create(
            disciplina=self.disc, numero=2, enunciado_md='Enunciado 2', gabarito='B',
            status=Questao.Status.CONCLUIDA,
        )
        self.topico = Topico.objects.create(
            disciplina=self.disc, nome='Princípios constitucionais', descricao='Base teórica', ordem=1,
        )
        ...
        TextoTopico.objects.create(
            topico=self.topico, status=TextoTopico.Status.CONCLUIDO,
