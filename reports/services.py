...
def gerar_relatorio(user, prompt, prova=None, disciplina=None, com_texto=True, formato="md"):
    ...
    qs = (
        ...
        .select_related("questao", "questao__disciplina")
        .order_by("questao__disciplina__nome", "questao__numero")
    )
    ...
    relatorio = Relatorio(
        user=user,
        titulo=titulo,
        tipo=Relatorio.Tipo.COMENTARIOS,
        prova=prova,
    ...
    if formato == "pdf":
        ...
        relatorio.arquivo_md.save(
            f"relatorio_{ts}.md", ContentFile(conteudo_md.encode("utf-8")), save=False
        )
...
def gerar_relatorio_topicos(user, prova=None, disciplina=None, formato="md"):
    ...
    qs = (
        ...
        .select_related("disciplina", "texto")
        .prefetch_related("questoes")
        .order_by("disciplina__nome", "ordem", "id")
    )
    if disciplina is not None:
    ...
    relatorio = Relatorio(
        user=user,
        titulo=titulo,
        tipo=Relatorio.Tipo.TOPICOS,
        prova=prova,
        disciplina=disciplina,
        com_texto=True,
        num_questoes=len(topicos),
    )
    ...
    if formato == "pdf":
        ...
        relatorio.arquivo_pdf.save(
            f"relatorio_topicos_{ts}.pdf", ContentFile(pdf_bytes), save=False
        )
    else:
        conteudo_md = _montar_markdown_topicos(titulo, topicos)
        relatorio.arquivo_md.save(
            f"relatorio_topicos_{ts}.md", ContentFile(conteudo_md.encode("utf-8")), save=False
        )
...
def _montar_markdown(titulo, prompt, resultados, com_texto):
    ...
    partes = [
        f"# {titulo}",
        "",
        f"> Prompt: {prompt.nome} · {total} {_plural(total, 'questão', 'questões')}",
        "",
    ]
...
def _montar_html(titulo, prompt, resultados, com_texto, md):
    ...
    for r in resultados:
        ...
        partes.append(
            f'<div class="corpo">{md.markdown(r.resultado_md or "", extensions=["extra"])}</div>'
        )
        partes.append("</div>")
...
def _montar_html_topicos(titulo, topicos, md):
    ...
    for t in topicos:
        ...
        partes.append(
            f'<div class="corpo">{md.markdown(t.texto.texto_md or "", extensions=["extra"])}</div>'
        )
        numeros = ", ".join(str(q.numero) for q in t.questoes.all())
        if numeros:
